...
function pick(obj, path, ret) {
    ...
    if (obj && len > 1) {
        ...
        let next = ret[key];
        if (isObject(obj)) {
            return pick(obj, path, next);
        }
        return obj.every((item, index)=> {
            let currentPath = JSON.parse(JSON.stringify(path))
            return pick(item, currentPath, next[index] || (next[index] = {}));
        });
    } else if (obj) {
        ret[key] = obj;
    } else {
...
function objFullPaths(obj) {
    function getPath(obj, path) {
        ...
        if (next && isObject(next) && !isEmptyObject(next)) {
            ...
            return getPath(next[0], path + '.' + key + '[]');
        } else if (key) {
            delete obj[key];
            return path + '.' + key;
        }
...
function filterPaths(fullPaths, paths, mode) {
    ...
    let brothersPathSet = new Set(brothersPaths);

    return fullPaths.filter(fullPath=> {
        return matchedPathSet.has(fullPath) || !hasPath(brothersPathSet, fullPath);
    });
}

/**
 * search path in a deep mode
 * @param pathSet
 * @param path
 * @returns {boolean}
 */
